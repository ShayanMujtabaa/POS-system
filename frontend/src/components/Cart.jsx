...
const Cart = () => {
  ...
  const handleIncrementQuantity = (id) => {
    console.log(`handling increment quantity for :${id}`)
    incQuantity(id);
  };
  ...
  const handleRefund = async (e) => {
    ...
    try {
      ...
      console.log("Error while processing refund: ", error.message);
  ...
  return (
    <div className="w-full">
      <h1 className="text-white mb-4 text-2xl sm:text-2xl lg:text-3xl lg:leading-normal font-extrabold">
        CART
      </h1>
      {cartItems.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className="bg-white border border-purple-500 p-2 pl-4 mb-2 flex justify-between items-center"
          >
            <div className="items-center gap-x-1.5">
              <div className="flex mb-2">
                <h4 className="text-base font-semibold mr-2">{item.name}</h4>
                <h4 className="text-sm justify-self-center mt-0.5">
                  @Rs. {item.price}
                </h4>
              </div>
              <div className="flex mb-1">
                <div className="relative flex items-center max-w-[7rem]">
                  <button
                    className="bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-e-sm p-2.5 h-9
            ...
            <button
              className="text-red-400 px-2 py-2 hover:text-red-700 transition-colors duration-300"
      ...
      <div className="flex justify-between mt-3">
        ...
        <div className="w-2/3 pl-2 pb-2 bg-[#60e0a6] border border-purple-800">
          ...
          <p className="text-left text-xl">
            {totalAmount(Total, Discount, Tax)}
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-start">
        ...
        <button
          className="bg-green-600 mx-1 text-white p-2 rounded w-1/2 hover:bg-green-700 transition-colors duration-300"
          onClick={null}
        >
          Put on Hold
        </button>
      ...
      {RetrievePopup && <HeldCartsList setRetrievePopup={setRetrievePopup} />}
    </div>
  );
};
